...
$select_str = $se_busca
  ? /* búsqueda */
  "select * from facturas where idfacturas = '{$_POST["dato"]}';"
  : /* select */
...
<head>
  ...
  <title>Facturas - Kriptotek</title>
...
<body>
      ...
      <div class="overx-auto">
        <table>
          ...
          <tbody>
            ...
            <!-- Mientras que la variable no sea nula, o seam, que no se quede sin filas... -->
            <tr>
              <!-- ...crearemos una fila tr... -->
              <?php for ($i = 0; $i < mysqli_num_fields($select); $i += 1) { ?>
              <!-- ...donde por cada columna... -->
              <td class="<?php echo $i == $primaryKeyIndex ? "p-key" : ""; ?>">
                <!-- ...crearemos una fila... -->
                <?php echo $campo[$i]; ?>
                <!-- ...que contendrá el valor del campo- -->
              </td>
              <?php } ?>
            </tr>
            <?php } ?>
          ...
          <form action="alta1.php" method="post" id="altaForm" class="alta-form">
            <tr class="alta">
              ...
              <td>
                <input type="number" name="precio" id="precio" form="altaForm" placeholder="Precio" require>
              </td>
              <td>
                <input type="datetime-local" name="fechahora" id="fechahora" form="altaForm">
              </td>
              <td>
                <input type="text" name="servicio" id="servicio" placeholder="Servicio" form="altaForm">
                <input type="hidden" name="table" id="table" form="altaForm" value="<?php echo $tabla; ?>">
              </td>
            </tr>
          </form>
        </table>
        <span class="aclaracion"><i>*Antes de insertar la factura, debe de comprobar que el e-mail del cliente está
  ...
  <form action="baja1.php" method="post" class="form-baja esconder-mensaje">
    ...
    <div class="mensaje">
      <h1>¿Está seguro que quiere borrar esta factura?</h1>
      <div class="buttons">
